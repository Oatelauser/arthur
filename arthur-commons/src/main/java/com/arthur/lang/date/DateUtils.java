...
import com.arthur.lang.utils.StringUtils;
...
import static com.arthur.lang.FastString.*;
...
public abstract class DateUtils {
	...
	public static Date parseDate(String str, String format, ZoneId zoneId) {
		...
		switch (format) {
			case "yyyy-MM-dd'T'HH:mm:ss" -> {
				long millis = parseMillis19(str, zoneId, DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH_T);
				return new Date(millis);
			}
			...
			case "yyyy/MM/dd HH:mm:ss" -> {
				long millis = parseMillis19(str, zoneId, DateTimeFormatPattern.DATE_TIME_FORMAT_19_SLASH);
				return new Date(millis);
			}
			case "dd.MM.yyyy HH:mm:ss" -> {
				long millis = parseMillis19(str, zoneId, DateTimeFormatPattern.DATE_TIME_FORMAT_19_DOT);
				return new Date(millis);
			}
			case "yyyy-MM-dd" -> {
				long millis = parseMillis10(str, zoneId, DateTimeFormatPattern.DATE_FORMAT_10_DASH);
				return new Date(millis);
			}
			case "yyyy/MM/dd" -> {
				long millis = parseMillis10(str, zoneId, DateTimeFormatPattern.DATE_FORMAT_10_SLASH);
				return new Date(millis);
			}
			case "yyyyMMdd" -> {
				DateTimeFormatter formatter;
				formatter = DateFormatterUtils.ofPattern(format);
	...
	public static long parseMillis(String str, ZoneId zoneId) {
		...
		if (len == 19) {
			millis = parseMillis19(str, zoneId);
		} else if (len > 19
		...
		} else if ((c0 == '-' || c0 >= '0' && c0 <= '9') && StringUtils.isNumber(str)) {
			...
			LocalDateTime ldt = parseLocalDateTime(str, 0, len);
	...
	/**
	 * yyyy-M-ddTHH:mm:ss yyyy-M-dd HH:mm:ss yyyy-MM-dTHH:mm:ss yyyy-MM-d HH:mm:ss
	 * yyyy-MM-ddTH:mm:ss yyyy-MM-dd H:mm:ss yyyy-MM-ddTHH:m:ss yyyy-MM-dd HH:m:ss
	 * yyyy-MM-ddTHH:mm:s yyyy-MM-dd HH:mm:s
	 */
	...
	static long parseMillis19(
		...
		char y0, y1, y2, y3, m0, m1, d0, d1, h0, h1, i0, i1, s0, s1;
		if (pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH) {
			if (c4 != '-' || c7 != '-' || c10 != ' ' || c13 != ':' || c16 != ':') {
				throw new DateTimeParseException("illegal input", str, 0);
			}
			y0 = c0;
			...
			s0 = c17;
			s1 = c18;
		} else if (pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH_T) {
			...
			s0 = c17;
			s1 = c18;
		} else if (pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_SLASH) {
			if (c4 != '/' || c7 != '/' || c10 != ' ' || c13 != ':' || c16 != ':') {
				throw new DateTimeParseException("illegal input", str, 0);
			}
			y0 = c0;
			...
			s0 = c17;
			s1 = c18;
		} else if (pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_DOT) {
	...
	public static long parseMillis19(String str, ZoneId zoneId) {
		...
		if (STRING_CODER != null && STRING_VALUE != null
			&& STRING_CODER.applyAsInt(str) == 0) {
			byte[] chars = STRING_VALUE.apply(str);
			if (chars.length != 19) {
				throw new DateTimeParseException("illegal input " + str, str, 0);
			}
	...
	public static String formatYMDHMS19(Date date, ZoneId zoneId) {
		...
		if (STRING_CREATOR != null) {
			...
			return STRING_CREATOR.apply(bytes, StandardCharsets.ISO_8859_1);
		}
	...
	public static String formatYMD8(long timeMillis, ZoneId zoneId) {
		...
		if (STRING_CREATOR != null) {
			...
			str = STRING_CREATOR.apply(bytes, StandardCharsets.ISO_8859_1);
		} else {
	...
	public static String format(Date date, String format) {
		...
		DateTimeFormatter formatter = DateFormatterUtils.ofPattern(format);
	...
	public static String format(ZonedDateTime zdt, String format) {
		...
		switch (format) {
			case "yyyy-MM-dd HH:mm:ss": {
				...
				int minute = zdt.getMinute();
				int second = zdt.getSecond();
				return format(year, month, dayOfMonth, hour, minute, second,
					DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH);
			}
			case "yyyy-MM-ddTHH:mm:ss":
			case "yyyy-MM-dd'T'HH:mm:ss": {
				int hour = zdt.getHour();
				...
				int second = zdt.getSecond();
				return format(year, month, dayOfMonth, hour, minute, second,
					DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH_T);
			}
			case "yyyy-MM-dd":
	...
	public static String format(
		...
		if (STRING_CREATOR != null) {
			...
			if (pattern == DateTimeFormatPattern.DATE_FORMAT_10_DOT) {
				...
				byte separator = (byte) (pattern == DateTimeFormatPattern.DATE_FORMAT_10_DASH ? '-' : '/');
				bytes[0] = (byte) y0;
				bytes[1] = (byte) y1;
				bytes[2] = (byte) y2;
				bytes[3] = (byte) y3;
		...
		if (pattern == DateTimeFormatPattern.DATE_FORMAT_10_DOT) {
			...
			char separator = (pattern == DateTimeFormatPattern.DATE_FORMAT_10_DASH ? '-' : '/');
			chars[0] = (char) y0;
			chars[1] = (char) y1;
			chars[2] = (char) y2;
			chars[3] = (char) y3;
	...
	public static String format(long timeMillis, DateTimeFormatPattern pattern) {
		...
		if (pattern == DateTimeFormatPattern.DATE_FORMAT_10_DASH || pattern == DateTimeFormatPattern.DATE_FORMAT_10_SLASH
			|| pattern == DateTimeFormatPattern.DATE_FORMAT_10_DOT) {
	...
	static String format(
		...
		if (STRING_CREATOR != null) {
			...
			if (pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_DOT) {
				...
				char separator = pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_DASH ? ' ' : 'T';
				char dateSeparator = pattern == DateTimeFormatPattern.DATE_TIME_FORMAT_19_SLASH ? '/' : '-';
				bytes[0] = (byte) y0;
				bytes[1] = (byte) y1;
				bytes[2] = (byte) y2;
				bytes[3] = (byte) y3;
	...
	public static String toString(long timeMillis, boolean timeZone, ZoneId zoneId) {
		...
		if (millislen > 0) {
			...
			if (millis < 10) {
				IOUtils.writeToByteArray(millis, 19 + millislen, bytes);
			} else {
				if (millis % 100 == 0) {
					IOUtils.writeToByteArray(millis / 100, 19 + millislen, bytes);
				} else if (millis % 10 == 0) {
					IOUtils.writeToByteArray(millis / 10, 19 + millislen, bytes);
				} else {
					IOUtils.writeToByteArray(millis, 19 + millislen, bytes);
				}
			}
		...
		if (timeZone) {
			...
			if (offsetTotalSeconds == 0) {
				...
				IOUtils.writeToByteArray(offsetAbs, 19 + millislen + 3, bytes);
				...
				IOUtils.writeToByteArray(offsetMinutes, 19 + millislen + zonelen, bytes);
			}
		}
