...
public class SentinelInvocationHandler implements InvocationHandler {
	...
	public SentinelInvocationHandler(Target<?> target, Map<Method, MethodHandler> dispatch,
			...
			if (methodMetadata == null) {
				...
				try {
					...
					if (fallbackFactory != null) {
						...
						/*
						 * 如果是GenericServerResponse，则触发自动降级
						 */
						if (GenericServerResponse.class.isAssignableFrom(method.getReturnType())) {
							return WebServerResponse.ofGenericStatus(HttpStatus.INTERNAL_SERVER_ERROR, ex.getLocalizedMessage());
						}
