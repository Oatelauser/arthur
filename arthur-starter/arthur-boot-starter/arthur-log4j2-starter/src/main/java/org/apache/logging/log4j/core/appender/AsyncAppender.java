...
/**
 * 优化{@link #append(LogEvent)}方法，避免日志线程Block，copy by {@link AsyncAppender}
 * <p>
 * 1.日志事件入队前避免触发异常堆栈类解析、加载操作
 * <br/>
 * 2.日志队列满时Appender忽略该日志
 */
...
public final class AsyncAppender extends AbstractAppender {
	...
	private AsyncAppender(final String name, final Filter filter, final AppenderRef[] appenderRefs,
			  ...
			  final boolean printDebugInfo, final BlockingQueueFactory<LogEvent> blockingQueueFactory,
	...
	public void append(final LogEvent logEvent) {
		...
		// 创建日志快照，避免解析、加载异常堆栈类
		final Log4jLogEvent memento = new Log4jLogEvent.Builder(logEvent)
		...
		if (!transfer(memento)) {
			if (blocking) {
				...
				if (printDebugInfo) {
					this.error("Appender " + getName() + " is unable to write primary appenders. queue is full");
				}
	...
	public static AsyncAppender createAppender(final AppenderRef[] appenderRefs, final String errorRef,
											   ...
											   final String name, final boolean includeLocation, final boolean printDebugInfo,
											   final Filter filter, final Configuration config, final boolean ignoreExceptions) {
		if (name == null) {
		...
		return new AsyncAppender(name, filter, appenderRefs, errorRef, size, blocking, ignoreExceptions,
			shutdownTimeout, config, includeLocation, printDebugInfo, new ArrayBlockingQueueFactory<LogEvent>(), null);
	}
	...
	public static class Builder<B extends Builder<B>> extends AbstractFilterable.Builder<B>
		...
		public Builder setPrintDebugInfo(final boolean printDebugInfo) {
			this.printDebugInfo = printDebugInfo;
			return this;
		}
		...
		public AsyncAppender build() {
			return new AsyncAppender(name, getFilter(), appenderRefs, errorRef, bufferSize, blocking, ignoreExceptions,
				shutdownTimeout, configuration, includeLocation, printDebugInfo, blockingQueueFactory, getPropertyArray());
		}
	}
